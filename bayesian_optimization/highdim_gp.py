...
def eval_all_metrics(
    ...
    if eval_logfile is None:
        eval_logfile = f"eval_rbf_dim{dim_problem}_all_metrics.log"
    else:
    ...
    with torch.no_grad():
        ...
        for batch in tqdm.tqdm(eval_batches, ascii=True):
            ...
            if model_type in ["np", "anp", "bnp", "banp"]:
                outs = model.predict(batch.xc, batch.yc, batch.xt, num_samples=num_bootstrap)
            else:
            ...
            scoring_rule = uct.metrics.get_all_scoring_rule_metrics(
                mean, std, yt, resolution=99, scaled=True, verbose=False
            )
    ...
    line = f'[eval] {model_type}: rbf_dim{dim_problem} all_metrics '
...
def main():
    ...
    parser.add_argument('--mode', choices=['train', 'eval', 'eval_all_metrics', 'generate'], default='train')
    ...
    if args.mode == "train":
        ...
        if not osp.isdir(datasets_path):
            os.makedirs(datasets_path)
